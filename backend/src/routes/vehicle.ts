...
import lookup from "@/lib/vehicles/fleet/lookup.json" with { type: "json" };
import trolleybusByPlate from "@/lib/vehicles/fleet/trolleybus-by-plate.json" with { type: "json" };
import tramByInventory from "@/lib/vehicles/fleet/tram-by-inventory.json" with { type: "json" };
import tramByPlate from "@/lib/vehicles/fleet/tram-by-plate.json" with { type: "json" };
...
export async function registerVehicleRoutes(app: FastifyInstance) {
  app.get("/vehicle/search", async (request, reply) => {
    ...
    const parsed = vehicleSearchSchema.safeParse({ q: query.q ?? "" });
    if (!parsed.success) {
    ...
    for (const rec of Object.values(trolleyLookup)) {
      if (results.length >= 20) break;
      const plateNorm = normalizePlate(rec.plate);
      if (plateNorm.includes(qPlate) || rec.plate.toUpperCase().includes(qUpper)) {
        pushFleetRecord(rec);
      }
    }
  ...
  app.get<{ Params: { inventoryId: string }; Querystring: { plate?: string } }>(
    "/vehicle/:inventoryId",
    async (request, reply) => {
      const ip = getClientIp(request);
      ...
      const id = parseInt(request.params.inventoryId, 10);
      const plate = request.query.plate ?? null;

      if (Number.isNaN(id) || id < 1 || id > 8999 || !hasFleetRecord(plate, id)) {
        return reply.status(404).send({ error: ro.lookup.notFound });
      }

      const fleetInfo = resolveFleetInfo(plate, id);
      const liveMaps = await getLiveVehicleMaps();
      const vehicle = await buildVehicleResultWithAc(
        id,
        liveMaps,
        fleetInfo?.plate ?? plate
      );

      return { vehicle, fleetInfo };
    }
  );
}
